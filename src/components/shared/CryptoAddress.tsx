...
export default function CryptoAddress({ currency, address, qrUrl, comingSoon = false }: CryptoAddressProps) {
  const [copied, setCopied] = useState(false);
  const [showFullAddress, setShowFullAddress] = useState(false);
  const info = currencyInfo[currency];

  const handleCopy = async () => {
    if (comingSoon || !address) return;
  ...
  return (
    <div className="flex flex-col items-center w-full py-4">
      ...
      <div 
        ...
        onMouseEnter={() => !comingSoon && trackEvent('crypto_qr_view', { currency })}
      >
        <div className={`w-40 h-40 bg-white p-4 rounded-2xl shadow-[0_0_40px_rgba(255,255,255,0.1)] flex items-center justify-center transition-all duration-300 mx-auto relative z-10 border-2 border-neutral-200 ${comingSoon ? 'opacity-50' : 'group-hover:scale-105 group-hover:shadow-[0_0_50px_rgba(255,255,255,0.15)]'}`}>
          {comingSoon ? (
            <div className="w-full h-full border-2 border-dashed border-neutral-300 rounded-xl flex flex-col items-center justify-center text-neutral-400 gap-3 bg-gradient-to-br from-neutral-50 to-neutral-100">
              ...
              <span className="text-xs font-bold tracking-widest opacity-50 text-neutral-500">COMING SOON</span>
            </div>
          ) : qrUrl ? (
      ...
      <div className="w-full max-w-md mx-auto">
        ...
        {comingSoon ? (
          <div className="bg-neutral-950 rounded-xl p-6 border-2 border-dashed border-neutral-800">
            <div className="flex flex-col items-center justify-center gap-3">
              ...
              <p className="text-xs text-neutral-500 text-center leading-relaxed">
                Direct {info.name} addresses will be available soon. Use Coinbase Commerce above for now.
              </p>
            </div>
          </div>
        ) : (
